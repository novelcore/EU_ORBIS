...
## Data
This repository contains the implementation of ORBIS Project in **Polis** dataset. 
The dataset consists of user comments in a specific Polis conversation.
Given the unstructured nature of the POLIS dataset compared to Bcause, we introduced an additional step to classify comments into "Against" and "In favor" categories. 
<br><br>
## Clustering
...
<img src="images/ORBIS_POLIS_KG.png" height="900">

Here is the nodes with their propertirs in detail:

- **QUERY**
  - *id (STRING):* Unique identifier for the `QUERY` node.
  - *author_id (STRING):* Unique identifier for the author of the content.
  - *title (STRING):* The title of the query.
  - *description (STRING):* A brief tagline or description for the query.
...
<img src="images/Entity_Linking.png" alt="KG Diagram" width="600">

## Improvements

1. In Version 0.2, we replace k-means with Fuzzy c-means (FCM). The advantages of this approach are:

    - *Better Handling of Overlapping Clusters:* FCM is well-suited for datasets where clusters overlap, as it considers partial membership of points in different clusters.

    - *Flexibility:* The fuzzy membership function in FCM allows fine-tuning using the fuzziness parameter, providing greater control over clustering outcomes.

    - *Robustness to Noise and Outliers:* By distributing membership among clusters, FCM can be more tolerant to noise and outliers compared to K-Means, which can be heavily affected by outliers as they distort the mean.

    - *Granular Insights:* FCM provides a membership matrix that indicates the probability of each point belonging to each cluster, giving more detailed insights than K-Means' simple cluster assignment.

    - *Continuous Space Consideration:* FCM is not limited to discrete assignments and can adapt better to continuous data distributions.

2. *Opinion classification component*: This component incorporates a polarity detection mechanism, leveraging an LLM to classify each piece of feedback into one of two 
sentiment groups: "In favor" and "Against", based on the feedback's stance relative to the discussion topic, which is provided through its title and description.
...
- Version 0.1
  - Retrieve data from BCause
  - Implemented functionalities: Clustering, summarization, keyphrase extraction, title generation, KG enrichment (NER, wiki data linking, triplet extraction)
