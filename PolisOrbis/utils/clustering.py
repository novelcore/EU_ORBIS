...
def find_optimal_clusters(data: np.ndarray, min_clusters: int = 2, max_clusters: int = 10) -> int:
    """
    Finds the optimal number of clusters for Fuzzy C-Means clustering using the Fuzzy Partition Coefficient (FPC).
    ...
    Returns:
    int: The optimal number of clusters based on the FPC score.
    """
    if data.ndim != 2:
...
def clustering(embeddings: np.ndarray = None, print_stat: bool = True, tuning: bool = True) -> Tuple[np.ndarray, int, np.ndarray]:
    """
    Perform clustering using Fuzzy C-Means (FCM) algorithm.
    ...
    Returns:
        Tuple[np.ndarray, int, np.ndarray]: Tuple containing cluster labels, total number of clusters found, and cluster centers.
    """
    ...
    if tuning:
        ...
        num_clusters = find_optimal_clusters(embeddings, min_clusters=2, max_clusters=15)
    else:
        num_clusters = 3  # Default to 3 clusters if no tuning

    # Perform Fuzzy C-Means clustering
    cntr, u, _, _, _, _, fpc = fuzz.cluster.cmeans(
        embeddings.T, c=num_clusters, m=2, error=0.005, maxiter=1000, init=None
    )
    
    # Get hard cluster labels by assigning each point to the cluster with the highest membership
    labels = np.argmax(u, axis=0)

    if print_stat:
        print("\033[92m[INFO]\033[0m" + f" Fuzzy Partition Coefficient (FPC): {fpc:.3f}")
        print("\033[92m[INFO]\033[0m" + f" Total Clusters found: {num_clusters}")

    return labels, num_clusters, cntr
...
def clustering_and_preprocess(data: Dict[str, Any]=None, topic: str=None, embs_model: str = "jinaai/jina-embedding-t-en-v1", cluster_threshold: int=6, save_model: bool=False) -> Dict[str, Any]:
    """
    Perform clustering and preprocessing on the input DataFrame of Positions and Arguments.

    Args:
        data (Dict[str, Any]): The JSON object containing polis data.
        topic (str): The topic of polis discussion.
    ...
    # Store pall osts in a Dataframe
    df = pd.DataFrame([entry["txt"] for entry in data if "txt" in entry], columns=["comment-body"])
    # df = pd.DataFrame([post.get('comment-body') for post in data['posts']], columns=['comment-body'])
    query = topic
    position_id = data[0].get("conversation_id", None)
    
    # Clean Posts
    target_column = 'comment-body'
    ...
    posts = df[target_column].tolist()
        
    if(len(posts) != 0):

        # Classify each post in Against or in Favor
        against_posts, in_favor_posts = opinion_classification(posts, query, method="gpt-35-turbo")
        
        for idx, posts in enumerate([against_posts, in_favor_posts]):
            
            results = {}

            # Here we assume that we perform clustering only for more than 6 arguments
            if(len(posts) >= cluster_threshold):
                # print(len(new_data))

                # Create embeddings
                embeddings = sent_model.encode(posts, show_progress_bar=False, device = device)
                ...
                umap_model = umap.UMAP( n_neighbors=30,
                                        n_components=10 if len(posts) >= 15 else 5 if len(posts) > 6 else 3,
                                        metric='cosine',
                                        random_state=42,
                                        verbose=False
                                    )
                ...
                if(save_model):
                    model_path = "Data/models/" + str(query) + ".pkl"
                ...
                for cluster_label, data_text in zip(soft_clusters, posts):
                    cluster_dict[cluster_label].append(data_text)

                # Enrich with cluster summaries, keyphrases, and titles.
                ...
                for data_text in posts:
                    cluster_dict[0].append(data_text)

                # Enrich with cluster summary, keyphrases, and title.
                results = clustering_details(
                                                cluster_dict=cluster_dict,
                                                subject=query,
                                                num_clusters=total_clusters,
            ...
            if(results != {}):
                ...
                results['Arguments_Type'] = 'In_Favor' if idx == 1 else 'Against'
                all_results.append(results)
    
    # Iterate through each post in the JSON data
    for post in data:
        argument_text = post.get("txt")
            ...
            for cluster_id, cluster_values in clusters.items():
                for argument in cluster_values:
                    if (get_clean(argument_text, special_characters=True, lowercase=True) == argument):
                        new_cluster_id = str(position_id) + "_" + str(result.get('Arguments_Type')) + "_" + str(cluster_id)
                        post['cluster_id'] = new_cluster_id
                        post['discussion_type'] = str(result.get('Arguments_Type'))
