...
def text2KG(data: Dict[str, Any]=None, topic_metadata: Dict[str, Any]=None, neo4j_settings: Dict[str, str]=None, clean_knowledge_graph: bool=True):
    """
    ...
    Args:
        ...
        topic_metadata (Dict[str, Any]): The JSON object containing bcause topic data.
    ...
    # Create Suject node
    query = f"""MERGE (s:SUBJECT {{id:"{topic_metadata['conversation_id']}", author_id:"{topic_metadata['ownername']}", title:"{topic_metadata['topic']}", tagline:"{topic_metadata['description']}"}})"""
    graph.query(query)
    ...
    for item in tqdm(data["posts"]):
        ...
        if item['discussion_type'] not in ["Against", "In_Favor"]: continue
                                        
        if item['discussion_type'] == "Against":   
            node_type = "AGAINST"
        else:
            node_type = "INFAVOR"
